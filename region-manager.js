...
(function() {
    ...
    function notifyTerrainSamplerPatch(detail = {}) {
        ...
        if (detail.reset) {
            navigationState.pendingPatch = detail.patch ? {
                ...detail.patch
            } : null;
        ...
        const aggregated = navigationState.pendingPatch ? {
            ...navigationState.pendingPatch
        } : (detail.patch ? {
            ...detail.patch
        } : null);
        const payload = {
    ...
    function consumeTerrainSamplerPatch() {
        ...
        const patch = navigationState.pendingPatch ? {
            ...navigationState.pendingPatch
        } : null;
    ...
    function buildRegionLodProfile(regionId) {
        ...
        return {
            version: DEFAULT_LOD_CONFIG.version,
            assets: baseAssets
        };
    }
    ...
    function normalizeRegionAssets(assets, regionId) {
        ...
        const push = (item, fallbackType) => {
            const normalized = normalizeAssetSpec(fallbackType ? {
                ...item,
                type: fallbackType
            } : item, regionId);
    ...
    function normalizeInteriorTrigger(source) {
        ...
        if (Number.isFinite(source.radius) || Number.isFinite(source.r)) {
            const center = toVec3(source.center ?? source.position ?? source.origin ?? source, {
                x: 0,
                y: 0,
                z: 0
            });
        ...
        if (source.min || source.max) {
            min = toVec3(source.min, {
                x: 0,
                y: 0,
                z: 0
            });
            max = toVec3(source.max, {
                x: 0,
                y: 0,
                z: 0
            });
        } else if (source.from || source.to) {
            min = toVec3(source.from, {
                x: 0,
                y: 0,
                z: 0
            });
            max = toVec3(source.to, {
                x: 0,
                y: 0,
                z: 0
            });
        } else if (source.start || source.end) {
            min = toVec3(source.start, {
                x: 0,
                y: 0,
                z: 0
            });
            max = toVec3(source.end, {
                x: 0,
                y: 0,
                z: 0
            });
        ...
        if (!min || !max) {
            const center = toVec3(source.center ?? source.position ?? source.origin ?? {
                x: 0,
                y: 0,
                z: 0
            }, {
    ...
    function normalizeInteriorSet(def, regionId, index = 0) {
        ...
        const metadata = (def.metadata && typeof def.metadata === "object") ? {
            ...def.metadata
        } : {};
        return {
            id,
    ...
    function normalizeInteriorPortal(def, regionId, index = 0) {
        ...
        const id = typeof rawId === "string" && rawId.trim() ? rawId.trim() : null;
        if (!id) return null;
        const center = toVec3(def.center ?? def.position ?? def.origin ?? def.point ?? {
            x: 0,
            y: 0,
            z: 0
        }, {
        ...
        const forward = normalizeVec3(def.normal ?? def.forward ?? def.direction ?? {
            x: 0,
            y: 0,
            z: 1
        }, {
        ...
        up = normalizeVec3(subtractVec3(up, scaleVec3(forward, dotVec3(up, forward))), {
            x: 0,
            y: 1,
            z: 0
        });
    ...
    function evaluateInteriorOcclusion(regionId, point = lastKnownPosition, opts = {}) {
        ...
        for (const setState of sets.values()) {
            ...
            applyInteriorSetState(id, setState, desired, {
                force: opts.force
            });
        }
        return true;
    ...
    function suspendInteriorOcclusion(regionId) {
        ...
        for (const setState of runtime.sets.values()) {
            if (setState.enabled) {
                applyInteriorSetState(id, setState, false, {
                    force: true,
                    silent: true,
                    reason: "region-exit"
                });
    ...
    function registerInteriorHints(regionId, hints) {
        ...
        if (activeRegionId === id) {
            evaluateInteriorOcclusion(id, lastKnownPosition, {
                force: true
            });
        }
        return normalized;
    ...
    function updateSpatialState(position = {}, opts = {}) {
        ...
        if (region) {
            ...
            evaluateInteriorOcclusion(region.id, point, {
    ...
    function applyRegion(region, opts = {}) {
        ...
        if (terrainApi && typeof terrainApi.setActiveRegion === "function") {
            try {
                terrainApi.setActiveRegion(region);
            } catch (err) {
        ...
        window.WorldUtils?.setTerrainStreamingRadius?.(streamRadius, {
            mode: "base"
        });
        ...
        evaluateInteriorOcclusion(region.id, lastKnownPosition, {
            force: true
        });
        return true;
    }
    ...
    function onRegionChange(cb) {
        ...
        if (activeRegionId && registry.has(activeRegionId)) {
            try {
                cb(registry.get(activeRegionId));
            } catch (err) {
                console.warn("[RegionManager] init listener failed", err);
            }
        }
    ...
    const API = {
        ...
        getDefaultLODConfig: () => ({
            version: DEFAULT_LOD_CONFIG.version,
            assets: clone(DEFAULT_LOD_CONFIG.assets)
        }),
        ...
        refreshInteriorOcclusion: (regionId) => {
            ...
            return !!evaluateInteriorOcclusion(targetId, lastKnownPosition, {
                force: true
            });
        },
        setInteriorSetState: (regionId, setId, enabled) => {
            ...
            applyInteriorSetState(targetId, state, enabled, {
                force: true
            });
            return true;
        }
