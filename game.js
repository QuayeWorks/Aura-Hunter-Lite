...
(function() {
    ...
    function createTerrainWorkerPool() {
        ...
        // inside createTerrainWorkerPool()
        function handleMessage(wrapper, event) {
            wrapper.busy = false;
            const msg = event?.data || {};
            const jid = msg.jobId;
            const job = state.pending.get(jid);
            if (!job) return;
            ...
            if (msg.error) {
                ...
                // DO NOT redeclare the name "result" anywhere else:
        ...
        function handleError(wrapper, err) {
            wrapper.busy = false;
            const failure = err instanceof Error ? err : new Error(String(err?.message || err));
            // Fail all currently running jobs on this worker (mirrors your queue policy)
            for (const [id, job] of state.pending.entries()) {
    ...
    function createDynamicTerrainAtlas(scene) {
        ...
        const width = tileSize * layers;
        const height = tileSize;
        const dynamicTexture = new BABYLON.DynamicTexture("terrainAtlasDynamic", {
            width,
            height
    ...
    function maybeLoadCompressedTerrainAtlas(scene) {
        ...
        ensureKtx2Support(scene.getEngine())
            .then((supported) => {
                ...
                return loadTextureAsync(terrainTextureState.compressedUrl, scene, {
                        invertY: false
                    })
                    .then((texture) => {
                        configureAtlasTexture(texture);
    ...
    const environment = {
        ...
        terrainSettings: {
            ...savedTerrainSettings,
            depthThresholds: {
                ...savedTerrainSettings.depthThresholds
            }
        },
    ...
    const interiorOcclusion = (() => {
        ...
        const ingestPayload = (group, payload, options = {}) => {
            ...
            for (const entry of list) {
                ...
                if (entry && typeof entry === "object") {
                    ...
                    if (entry.metadata) group.metadata = {
                        ...group.metadata,
                        ...entry.metadata
                    };
                    ...
                    if (Array.isArray(entry.onToggle)) {
                        entry.onToggle.forEach(cb => {
                            if (typeof cb === "function") group.callbacks.add(cb);
                        });
                    }
                    if (typeof entry.resolver === "function") group.resolvers.push(entry.resolver);
                    if (Array.isArray(entry.resolvers)) {
                        entry.resolvers.forEach(fn => {
                            if (typeof fn === "function") group.resolvers.push(fn);
                        });
                    }
                    if (entry.nodes || entry.meshes || entry.node || entry.mesh || entry.transform) {
                        ...
                        if (ingestPayload(group, nodes, {
                                ...options,
                                includeDescendants: entry.includeDescendants ?? options.includeDescendants
                            })) added = true;
                    }
            ...
            if (!options.silent && group.callbacks.size) {
                ...
                group.callbacks.forEach(cb => {
                    try {
                        ...
                        console.warn("[InteriorOcclusion] callback failed", err);
                    }
                });
            }
            return value;
        ...
        const resolveGroup = (group, options = {}) => {
            ...
            if (group.resolvers.length) {
                ...
                for (const resolver of resolvers) {
                    ...
                    try {
                        result = resolver({
                            group,
                            scene: group.scene || getActiveScene(),
                            options
                        }) || null;
                    } catch (err) {
            ...
            if ((added || group.nodes.size || group.particleSystems.size || group.lights.size || group.animationGroups.size) && !group.resolved) {
                group.resolved = true;
                applyGroupStateInternal(group, group.desiredState, {
                    force: true,
                    silent: true
                });
            }
        ...
        const setGroupState = (key, enabled, options = {}) => {
            ...
            if (options.metadata) group.metadata = {
                ...group.metadata,
                ...options.metadata
            };
        ...
        const registerGroup = (key, spec = {}) => {
            ...
            if (typeof spec.enabled === "boolean" && group.resolved) {
                applyGroupStateInternal(group, spec.enabled, {
                    force: true,
                    silent: true
                });
            }
        ...
        const withGroup = (key, cb) => {
            ...
            if (typeof cb === "function") {
                try {
                    ...
                    console.warn("[InteriorOcclusion] withGroup callback failed", err);
                }
            }
            return group;
        };
    ...
    function applyDynamicResolutionState({
        immediate = false
    } = {}) {
    ...
    function initializeAdaptiveQuality(sceneRef, engineRef, cameraRef) {
        ...
        if (adaptiveQuality.pipeline) {
            try {
                adaptiveQuality.pipeline.dispose();
            } catch (err) {}
        ...
        if (adaptiveQuality.optimizer) {
            try {
                adaptiveQuality.optimizer.stop();
            } catch (err) {}
        ...
        if (BABYLON.SceneOptimizerOptions) {
            try {
                ...
                adaptiveQuality.optimizer.onSuccessObservable?.add(() => {
                    adaptiveQuality.optimizerRunning = false;
                });
                adaptiveQuality.optimizer.onFailureObservable?.add(() => {
                    adaptiveQuality.optimizerRunning = false;
                });
            } catch (err) {
        ...
        setQualityLevel(adaptiveQuality.currentLevel, {
            force: true
        });
    }
    ...
    function ensureProfilerHudConfigured(hudApi) {
        ...
        hudApi.configureProfilerOverlay({
            ...
            onChunkRadiusChange: (value) => {
                if (Number.isFinite(value)) setTerrainStreamingRadius(value, {
                    mode: "manual"
                });
            },
            onChunkRadiusReset: () => setTerrainStreamingRadius(null, {
                mode: "manual"
            })
        });
    ...
    const INSTANCE_POOL = (() => {
        ...
        const ensureBaseMesh = (entry) => {
            ...
            if (entry.baseMesh && typeof entry.baseMesh.dispose === "function") {
                try {
                    entry.baseMesh.dispose(false, true);
                } catch (err) {
                    /* ignore */ }
            }
            entry.baseMesh = null;
        ...
        const convertToThin = (entry) => {
            ...
            for (const [id, record] of entry.instanceRecords.entries()) {
                nextMatrices.push({
                    id,
                    matrix: record.transform.matrix.clone()
                });
                if (record.mesh && typeof record.mesh.dispose === "function") {
                    try {
                        record.mesh.dispose();
    ...
    function ensureTreePrototypes(scene) {
        ...
        const {
            ...
            leavesMat,
            billboardMat
        } = getFallbackTreeMaterials(scene);
    ...
    const GameSettings = {
        ...
        setTerrainSettings(update) {
            ...
            const thresholds = update?.depthThresholds ?
                normalizeDepthThresholds({
                    ...current.depthThresholds,
                    ...update.depthThresholds
                }, current.depthThresholds) :
                {
                    ...current.depthThresholds
                };
            const merged = normalizeTerrainSettings({
                ...current,
                ...update,
            ...
            if (environment.terrain) {
                const terrainSettings = {
                    ...cloned,
                    depthThresholds: {
                        ...cloned.depthThresholds
                    }
                };
                environment.terrain.settings = terrainSettings;
                environment.terrain.depthThresholds = {
                    ...cloned.depthThresholds
                };
                const terrainApi = getTerrainApi();
                if (terrainApi?.setDepthThresholds) {
                    try {
                        terrainApi.setDepthThresholds(cloned.depthThresholds);
                    } catch (err) {
                        console.warn("[Terrain] Failed to apply updated depth thresholds", err);
                    }
                }
                initializeTerrainStreaming(environment.terrain, terrainSettings, {
    ...
    function ensureEnemySimulationHandle(enemy, plan) {
        ...
        const sim = enemy.__sim || (enemy.__sim = {
            state: SIM_STATE_ACTIVE,
    ...
    function disposeTerrain() {
        ...
        if (terrain.streaming) {
            releaseStreamingChunks(terrain.streaming, {
                disposeMeshes: true
            });
    ...
    function createTerrain(scene, options = {}) {
        ...
        if (atlasRects.length < expectedAtlasCount) {
            const filler = atlasRects.length ? atlasRects[atlasRects.length - 1] : {
                u0: 0,
                v0: 0,
                u1: 1,
                v1: 1
        ...
        const defaultRect = atlasRects.length ? atlasRects[0] : {
            u0: 0,
            v0: 0,
            u1: 1,
            v1: 1
        ...
        for (let layer = 0; layer < layers; layer++) {
            ...
            const faceUV = Array.from({
                length: 6
            }, () => new BABYLON.Vector4(rect.u0, rect.v0, rect.u1, rect.v1));
            ...
            if (useUnified) {
                ...
                if (mesh && typeof terrainApi.updateFromColumns === "function") {
                    ...
                    if (activeRegion && typeof terrainApi.setActiveRegion === "function") {
                        try {
                            terrainApi.setActiveRegion(activeRegion);
                        } catch (err) {
        ...
        initializeTerrainStreaming(environment.terrain, settings, {
            preserveOverride: true
        });
    ...
    function setChunkWorkerEnabled(enabled) {
        ...
        updatePerfSettings({
            workerEnabled: chunkWorkerEnabled
        });
    ...
    function recycleChunkMeta(terrain, chunk, {
        disposeMesh = false
    } = {}) {
    ...
    function recycleTerrainChunkMesh(terrain, chunk, {
        dispose = false
    } = {}) {
    ...
    function releaseStreamingChunks(streaming, {
        disposeMeshes = false
    } = {}) {
        ...
        for (const chunk of streaming.chunks) {
            if (!chunk) continue;
            recycleChunkMeta(terrain, chunk, {
                disposeMesh: disposeMeshes
            });
        }
    ...
    function queueChunkVoxelJob(streaming, chunk, terrain) {
        ...
        // Column indices for this chunk (spanX*spanZ)
        ...
        // Copy column indices safely (transferable if possible)
        ...
        // Chunk X/Z spans; never 0
        const spanX = (chunk.spanX | 0) || streaming.chunkSize || 1;
        const spanZ = (chunk.spanZ | 0) || streaming.chunkSize || 1;

        // Heights per column (meters). We’ll turn these into voxel IDs in the worker.
        ...
        // Layer setup (meters) → worker will convert to voxel depths
        const layerOffsets = Array.isArray(terrain.layerOffsets) ? terrain.layerOffsets.slice() : [];
        const layerThicknesses = Array.isArray(terrain.layerThicknesses) ? terrain.layerThicknesses.slice() : [];

        // Terrain materials atlas
        ...
        // --- LOD selection (Roblox-like: full detail within 15m, quadratic drop to 4km)
        const player = (terrain?.player) || (streaming?.player) || null;
        const cx = (chunk.bounds?.minX ?? 0) + (chunk.bounds?.maxX ?? 0) * 0.5;
        const cz = (chunk.bounds?.minZ ?? 0) + (chunk.bounds?.maxZ ?? 0) * 0.5;
        const px = player?.position?.x ?? streaming.lastPlayerPosition?.x ?? 0;
        const pz = player?.position?.z ?? streaming.lastPlayerPosition?.z ?? 0;
        const dx = px - cx;
        const dz = pz - cz;
        const dist = Math.sqrt(dx * dx + dz * dz);

        // LOD 0 = finest; grows ~quadratically; clamps so ~4km is culled by streaming anyway
        let lod = 0;
        if (dist > 15) {
            const t = Math.min(1, (dist - 15) / 4000); // 0..1 across 15m..4km
            lod = Math.min(5, Math.floor(1 + 8 * t * t)); // 1..5, quadratic
        }

        // payload to worker
        const payload = {
            // voxel column addressing
            indices: indicesCopy,
            colsX: Math.max(1, terrain.colsX | 0),
            colsZ: Math.max(1, terrain.colsZ | 0),
            halfX: terrain.halfX | 0,
            halfZ: terrain.halfZ | 0,

            // grid and material data
            spanX,
            spanZ,
        ...
        const transfer = [];
        if (heights?.buffer) transfer.push(heights.buffer);

        const job = pool.postJob(payload, transfer);
        if (job && typeof job.then === "function") {
            chunk.workerJob = job.then((result) => {
                // Robustness: ignore empty geometry to avoid Babylon warnings
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (!chunk.geometry && terrain.chunkGeometryCache?.has?.(chunk.index)) {
                ...
                if (cached) {
                    ...
                    applyTerrainChunkGeometry(terrain, chunk, cached, {
                        cache: false
                    });
                }
            } else if (chunk.geometry) {
                applyTerrainChunkGeometry(terrain, chunk, chunk.geometry, {
                    cache: false
                });
            }
    ...
    function applyStreamingRadius(streaming) {
        ...
        // when you compute base/max radius (you already do), keep <= 4000
        streaming.maxRadius = Math.min(streaming.maxRadius, 4000);
    ...
    function applyStoredChunkRadius(streaming = environment.terrain?.streaming) {
        ...
        if (desired == null) {
            if (currentOverride != null) {
                setTerrainStreamingRadius(null, {
                    mode: "manual",
                    forceImmediate: true
                });
            }
        ...
        setTerrainStreamingRadius(desired, {
            mode: "manual",
            forceImmediate: true
        });
    }
    ...
    function initializeTerrainStreaming(terrain, settings = {}, opts = {}) {
        ...
        if (adoptPrevious) {
            applyChunkDescriptor(streaming, {
                chunkSize,
                chunkCountX: previous.chunkCountX,
                chunkCountZ: previous.chunkCountZ,
                chunks: previous.chunks
            }, terrain);
            streaming.ready = true;
            scheduleTerrainRadiusUiUpdate();
            const pos = streaming.lastPlayerPosition || previous?.lastPlayerPosition || {
                x: 0,
                z: 0
            };
            ...
            const integrate = (descriptor) => {
                ...
                scheduleTerrainRadiusUiUpdate();
                const pos = streaming.lastPlayerPosition || {
                    x: 0,
                    z: 0
                };
                refreshChunkTargets(streaming, pos, true);
        ...
        return {
            ...
            step() {
                ...
                return {
                    ...
                    opsUsed: Math.max(1, limit - start)
                };
            }
        };
    }
    ...
    function refreshChunkTargets(streaming, position = {
        x: 0,
        z: 0
    }, force = false) {
    ...
    function ensureTerrainRadiusControl() {
        ...
        slider.addEventListener("input", () => {
            ...
            applyValue(val);
            setTerrainStreamingRadius(val, {
                mode: "manual"
            });
        });

        reset.addEventListener("click", () => {
            const radius = setTerrainStreamingRadius(null, {
                mode: "manual"
            });
    ...
    function setTerrainStreamingRadius(radius, opts = {}) {
        ...
        applyStreamingRadius(streaming);
        const center = opts.position || streaming.lastPlayerPosition || {
            x: 0,
            z: 0
        };
    ...
    function setTerrainStreamingBudget(update = {}) {
        ...
        return {
            ms: streaming.budgetMs,
            ops: streaming.budgetOps,
    ...
    function applyBrushMetrics(detail, elapsedMs) {
        ...
        if (detail?.worldX != null && detail?.worldZ != null) {
            ...
            const columns = gatherBrushColumns({
                x: detail.worldX,
                z: detail.worldZ
            }, radius);
            brush.lastPatch = {
    ...
    function resetTerrainBrushPatch() {
        ...
        let success = false;
        try {
            success = terrainApi.updateFromColumns(terrain, {
                columnIndices: columns
            });
        } catch (err) {
        ...
        if (bounds) {
            ...
            const patch = updateTerrainSamplerForColumns(terrain, bounds.minCX, bounds.maxCX, bounds.minCZ, bounds.maxCZ, {
                reset: true
            });
    ...
    function ensureCullingOverlay() {
        ...
        if (!overlay.root) {
            ...
            const renderCircle = BABYLON.MeshBuilder.CreateLines("culling-debug-render", {
                points: buildCirclePoints(1)
            }, scene);
            ...
            const cullCircle = BABYLON.MeshBuilder.CreateLines("culling-debug-cull", {
                points: buildCirclePoints(1.1)
            }, scene);
    ...
    function updateCullingOverlay() {
        ...
        if (!overlay.lastHudUpdate || changedRender || changedCull || changedRadius || now - overlay.lastHudUpdate > 400) {
            ...
            window.HUD?.setCullingOverlayState?.({
                enabled: true,
                metrics: overlay.lastMetrics
            });
        }
    }
    ...
    function disableCullingOverlay() {
        ...
        if (overlay.observer && scene?.onBeforeRenderObservable) {
            try {
                scene.onBeforeRenderObservable.remove(overlay.observer);
            } catch (err) {}
        }
        overlay.observer = null;
    ...
    function ensureRearProxyMeshes() {
        ...
        if (!state.root) {
            ...
            const marker = BABYLON.MeshBuilder.CreateSphere("rear-proxy-marker", {
                diameter: 0.6,
                segments: 12
            }, scene);
            marker.parent = root;
            ...
            const lineSystem = BABYLON.MeshBuilder.CreateLineSystem("rear-proxy-lines", {
                lines: linePoints
            }, scene);
    ...
    function updateRearDebugProxies() {
        ...
        if (state.linePoints?.length === 3) {
            ...
            BABYLON.MeshBuilder.CreateLineSystem(null, {
                lines: state.linePoints,
                instance: state.lineSystem
            });
        }
        const now = Date.now();
        if (now - (rearProxyState.lastHudSync || 0) > 600) {
            rearProxyState.lastHudSync = now;
            window.HUD?.setRearProxyState?.({
                enabled: true
            });
        }
    }
    ...
    function disableRearDebugProxies() {
        ...
        if (rearProxyState.observer && scene?.onBeforeRenderObservable) {
            try {
                scene.onBeforeRenderObservable.remove(rearProxyState.observer);
            } catch (err) {}
        }
    ...
    function disableSingleCamPerfTest() {
        if (singleCamPerfState.observer && scene?.onAfterRenderObservable) {
            try {
                scene.onAfterRenderObservable.remove(singleCamPerfState.observer);
            } catch (err) {}
        }
        singleCamPerfState.observer = null;
        singleCamPerfState.enabled = false;
        resetSingleCamPerfMetrics();
        window.HUD?.setSingleCamPerfState?.({
            enabled: false
        });
    }
    ...
    function scheduleTerrainSamplerFlush(terrain, sampler) {
        ...
        const flush = () => {
            ...
            const pendingPatch = sampler.pendingPatch ? {
                ...sampler.pendingPatch
            } : null;
    ...
    function initializeTerrainHeightSampler(terrain) {
        ...
        const sampler = ensureTerrainHeightSampler(terrain);
        if (!sampler) return;
        updateTerrainSamplerRegion(terrain, 0, sampler.vertexCountX - 1, 0, sampler.vertexCountZ - 1, {
            reset: true
        });
    }
    ...
    function removeTopBlock(columnIndex) {
        ...
        for (let layer = column.length - 1; layer >= 0; layer--) {
            ...
            if (terrainApi && isUnifiedTerrainActive() && typeof terrainApi.updateFromColumns === "function") {
                try {
                    terrainApi.updateFromColumns(terrain, {
                        columnIndex
                    });
                } catch (err) {
    ...
    function getFallbackTreeMaterials(scene) {
        ...
        const billboardTexture = new BABYLON.DynamicTexture("fallbackTreeBillboardTex", {
            width: 128,
            height: 192
        }, scene, false);
    ...
    function setEnvironmentLodEnabled(enabled) {
        ...
        scheduleProfilerHudSync();
        updatePerfSettings({
            lodEnabled: environment.lodEnabled
        });
    ...
    function recomputeTrainingEffects({
        silent = false
    } = {}) {
    ...
    function getFlowSnapshot() {
        ...
        const groups = computeFlowGroupTotals(state.flow).map(({
            key,
            label,
            value
        }) => ({
    ...
    function applyFlowPreset(index, {
        silent = false
    } = {}) {
    ...
    const FALLBACK_COSMETIC_CONFIG = {
        ...
        hair: [{
                ...
                label: "Buzz Cut",
                primaryColor: "#2f2f38",
                secondaryColor: "#3c3f4f"
            },
            {
                id: "windswept",
                label: "Windswept",
                primaryColor: "#1e2f6f",
                secondaryColor: "#2f478f"
            },
            {
                id: "scout_hat",
                label: "Explorer Hat",
                primaryColor: "#6a4d32",
                secondaryColor: "#8c6a3e"
            }
        ],
        outfits: {
            top: {
    ...
    const FALLBACK_COSMETIC_ANCHORS = {
        faces: {},
        hair: {}
    };
    const BASE_COSMETIC_ANCHORS = normalizeAnchorMap((window.RigDefinitions && window.RigDefinitions.COSMETIC_ANCHORS) || FALLBACK_COSMETIC_ANCHORS, {
        base: true
    });
    ...
    function persistCosmeticsSelection({
        selection = null,
        emit = true
    } = {}) {
    ...
    function normalizeAnchorMap(source, {
        base = false
    } = {}) {
        const buckets = {
            faces: {},
    ...
    function trimAnchorOverride(entry) {
        if (!entry) return null;
        const out = {
            headSocket: entry.headSocket || null
        };

        function pruneVec(source, {
            epsilon = 1e-5
        } = {}) {
    ...
    function loadSavedCosmeticAnchors() {
        if (cachedCosmeticAnchorStorage) {
            return normalizeAnchorMap(cachedCosmeticAnchorStorage, {
                base: false
            });
        }
        if (typeof localStorage === "undefined") {
            return normalizeAnchorMap({}, {
                base: false
            });
        }
        try {
            const raw = localStorage.getItem(COSMETIC_ANCHOR_STORAGE_KEY);
            if (!raw) return normalizeAnchorMap({}, {
                base: false
            });
            const parsed = JSON.parse(raw);
            const payload = parsed && typeof parsed === "object" ?
                ...
                null;
            if (!payload) return normalizeAnchorMap({}, {
                base: false
            });
            cachedCosmeticAnchorStorage = normalizeAnchorMap(payload, {
                base: false
            });
            return normalizeAnchorMap(cachedCosmeticAnchorStorage, {
                base: false
    ...
    function getCosmeticAnchorOverridesSnapshot() {
        return normalizeAnchorMap(cosmeticAnchorOverrides, {
            base: false
        });
    }

    function persistCosmeticAnchors({
        overrides = null,
        emit = true
    } = {}) {
        if (overrides) {
            cosmeticAnchorOverrides = normalizeAnchorMap(overrides, {
                base: false
            });
        }
        cachedCosmeticAnchorStorage = normalizeAnchorMap(cosmeticAnchorOverrides, {
            base: false
        });
        if (typeof localStorage !== "undefined") {
            try {
                localStorage.setItem(COSMETIC_ANCHOR_STORAGE_KEY, JSON.stringify({
                    version: 1,
                    overrides: cachedCosmeticAnchorStorage
                }));
            } catch (err) {}
        }
        if (emit) scheduleRuntimeSave();
    ...
    function mergeAnchorDelta(current, delta = {}) {
        ...
        if (delta && typeof delta === "object") {
            ...
            if (delta.localScale && typeof delta.localScale === "object") {
                base.localScale = base.localScale || {
                    x: 0,
                    y: 0,
                    z: 0
                };
    ...
    function adjustCosmeticAnchor(kind, id, delta = {}, {
        persist = true
    } = {}) {
        const normalized = normalizeAnchorKind(kind);
        ...
        if (persist) persistCosmeticAnchors({
            overrides: cosmeticAnchorOverrides,
            emit: true
        });
    ...
    function resetCosmeticAnchor(kind, id, {
        persist = true
    } = {}) {
        const normalized = normalizeAnchorKind(kind);
        if (!normalized || typeof id !== "string" || !id) {
    ...
    function saveCosmeticLoadout(selection = null) {
        return persistCosmeticsSelection({
            selection,
            emit: true
        });
    }

    function applyCosmeticLoadout(selection = null, {
        persist = true
    } = {}) {
    ...
    function setRigParameters(nextRig = undefined, {
        persist = true
    } = {}) {
    ...
    async function fetchRigDefault() {
        ...
        try {
            const res = await fetch("hxh_rig.xml", {
                cache: "no-cache"
            });
    ...
    function restoreInventoryFromSnapshot(snapshot) {
        ...
        if (Number.isInteger(data.activeHotbar)) {
            inventory.equip(data.activeHotbar, {
                silent: true
            });
        } else {
    ...
    function buildRuntimeSnapshot() {
        ...
        return {
            ...
            pools: {
                hp: {
                    cur: Number.isFinite(state.hp) ? state.hp : 0,
                    max: Number.isFinite(state.maxHP) ? state.maxHP : 0
                },
                nen: {
            ...
            timeStop: {
                active: !!state.timeStop,
                ultT: Number.isFinite(state.ultT) ? state.ultT : 0
            }
        };
    }
    ...
    function applyRuntimeSnapshot(snapshot = null, opts = {}) {
        ...
        if (snapshot.cosmetics && typeof snapshot.cosmetics === "object") {
            applyCosmeticLoadout(snapshot.cosmetics, {
                persist: false
            });
            applied = true;
        }
        ...
        if (snapshot.animations && typeof snapshot.animations === "object") {
            saveAnimationLibrarySnapshot(snapshot.animations, {
                emit: false
            });
            applied = true;
        }
        ...
        if (snapshot.region && typeof snapshot.region === "object") {
            ...
            if (targetRegion && window.RegionManager?.setRegion) {
                try {
                    window.RegionManager.setRegion(targetRegion, {
                        silent: true,
                        force: true
                    });
                } catch (err) {
    ...
    function spawnTargetDummy(options = {}) {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            configureEnemyForSimulation(enemy, {
                role: "dummy",
                meta: {
                    simulation: SIM_BEHAVIOR_SLEEP
                }
            });
            enemies.push(enemy);
            spawned += 1;
    ...
    function rebuildVowRuntime({
        keepWave = false
    } = {}) {
        const entries = Array.isArray(state.vows) ?
            state.vows.map(entry => Object.assign({}, entry, {
                broken: false,
                brokenReason: null
            })) :
            [];
    ...
    function disableRearDebugCamera() {
        ...
        if (owningScene && rear._dbgObserver) {
            try {
                owningScene.onBeforeRenderObservable.remove(rear._dbgObserver);
            } catch {}
        }
    ...
    async function startGame(ch, options = {}) {
        ...
        if (hudApi?.configureDevPanel) {
            hudApi.configureDevPanel({
                ...
                spawnDummy: count => spawnTargetDummy({
                    count
                }),
                refill: (target) => {
                    if (target === "hp") return refillResources({
                        hp: true,
                        nen: false
                    });
                    if (target === "nen") return refillResources({
                        hp: false,
                        nen: true
                    });
                    return refillResources({
                        hp: true,
                        nen: true
                    });
                },
                toggleRearView: (enabled) => {
                    if (enabled) {
                        const rearCam = enableRearDebugCamera({
                            w: 0.26,
                            h: 0.26
                        });
                        if (!rearCam) {
            ...
            scheduleTerrainRadiusUiUpdate();
            syncTerrainBrushHud();
            hudApi.setCullingOverlayState?.({
                enabled: cullingOverlayState.enabled,
                metrics: cullingOverlayState.lastMetrics
            });
            hudApi.setRearProxyState?.({
                enabled: rearProxyState.enabled
            });
            hudApi.setSingleCamPerfState?.({
                enabled: singleCamPerfState.enabled
            });
        ...
        if (runtimeSnapshot) {
            applyRuntimeSnapshot(runtimeSnapshot, {
                skipInventory: runtimeHasInventory
            });
    ...
    function createHumanoid(hex, rig = RIG, cosmeticPreset = DEFAULT_COSMETIC_SELECTION) {
        ...
        function applyAnchorTransform(node, anchor) {
            ...
            const rotationDeg = resolved.rotationDeg || {
                x: 0,
                y: 0,
                z: 0
            };
        ...
        function disposeAnimationBinding() {
            ...
            if (rigAnimation.binding?.group) {
                try {
                    rigAnimation.binding.group.dispose();
                } catch (err) {
                    /* ignore */ }
            }
        ...
        function rebuildAnimationBinding() {
            ...
            if (build && build.group) {
                ...
                rigAnimation.setFrame(targetFrame, {
                    loop: rigAnimation.loop !== false
                });
            } else {
                rigAnimation.frame = null;
                applyBasePose();
        ...
        function ensureFaceMaterial(faceId) {
            ...
            const size = 512;
            const texture = new BABYLON.DynamicTexture(`face-${id}`, {
                width: size,
                height: size
            }, scene, false);
        ...
        function instantiateAccessory(spec) {
            ...
            switch (spec.id) {
                ...
                case "earrings": {
                    const left = BABYLON.MeshBuilder.CreateSphere("acc-earringL", {
                        diameter: 0.1
                    }, scene);
                    ...
                    const right = BABYLON.MeshBuilder.CreateSphere("acc-earringR", {
                        diameter: 0.1
                    }, scene);
        ...
        function applyTop(id) {
            ...
            clothingRefs.sleeves.forEach(mesh => {
        ...
        function applyBottom(id) {
            ...
            clothingRefs.hips.forEach(mesh => {
                mesh.material = hipMat;
            });
            clothingRefs.thighs.forEach(mesh => {
                mesh.material = thighMat;
            });
            clothingRefs.shins.forEach(mesh => {
                mesh.material = shinMat;
            });
            return appliedId;
        }
    ...
    function openTrainingMenu() {
        ...
        const disposer = hudApi.openTrainingMenu({
            ...
            onClose: () => closeTrainingMenu({
                silent: false
            }),
            onComplete: (key) => {
                const outcome = upgradeTraining(key, {
                    silent: false
                });
    ...
    try {
        Object.assign(window.HXH, {
            ...
            removeTerrainCubeAtPoint,
            getTerrainStreamingRadius,
            setTerrainStreamingRadius,
            setTerrainStreamingBudget,
            ...
            advanceEnvironment,
            updateEnvironment,
            getFallbackTreeMaterials,
            createFallbackTree,
            applyTreeLOD,
            refreshAllTreeLods,
            setEnvironmentLodProfile,
            setEnvironmentLodEnabled,
            setInstanceRenderingMode,
            getInstanceRenderingMode,
            setGreedyMeshingEnabled,
